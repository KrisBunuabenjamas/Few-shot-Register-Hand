...
def _sort_hands_for_features(hands):
    """Keep two-hand feature slots stable from the viewer's left to right."""
    return sorted(hands, key=lambda hand: sum(p.x for p in hand) / len(hand))


def get_multi_hand_pairwise_distances(hands):
    """Create one-hand 441 or two-hand 1764 pairwise distance features."""
    sorted_hands = _sort_hands_for_features(hands)
    all_points = [p for hand in sorted_hands for p in hand]
    if len(all_points) == 21:
        return get_pairwise_distances(all_points)

    xs = [p.x for p in all_points]
    ys = [p.y for p in all_points]
    hand_sizes = [
        math.sqrt((hand[12].x - hand[0].x) ** 2 + (hand[12].y - hand[0].y) ** 2)
        for hand in sorted_hands
    ]
    span = math.sqrt((max(xs) - min(xs)) ** 2 + (max(ys) - min(ys)) ** 2)
    scale = max(span, sum(hand_sizes) / max(len(hand_sizes), 1), 1e-6)
...
def _feature_kind(features):
    if len(features) == 1764:
        return "pairwise_distances_42x42"
    if len(features) == 441:
        return "pairwise_distances_21x21"
    return f"pairwise_distances_{len(features)}"


def _landmark_payload_to_hands(lm_raw):
    if not lm_raw:
        return []
    ...
    hands = []
    for hand_raw in lm_raw[:2]:
        hands.append([types.SimpleNamespace(x=t[0], y=t[1]) for t in hand_raw])
    return hands
...
def knn_classify(test_features, samples, k, metric):
    dists = []
    for s in samples:
        train_features = np.array(s["features"])
        if train_features.shape != test_features.shape:
            continue
        d = distance_metric(test_features, train_features, metric)
...
def _class_counts(training_data):
    if not training_data:
        return {}
    counts = Counter(s.get("class") for s in training_data.get("samples", []))
...
def _resolve_training_path(path):
    if not path:
        return os.path.join(os.path.dirname(__file__), "asl_training.json")
...
def generate_frames():
    ...
    try:
        while True:
            ...
            confidence = 0.0
            votes = []
            hand_count = len(detection.hand_landmarks) if detection.hand_landmarks else 0
            feature_type = None

            if detection.hand_landmarks:
                hands = detection.hand_landmarks[:2]
                with state_lock:
                    state["latest_landmarks"] = [
                        [(p.x, p.y, p.z) for p in hand] for hand in hands
                    ]
                    state["latest_landmarks_at"] = time.time()
                h, w = frame.shape[:2]

                # Draw landmarks
                colors = [(0, 200, 255), (255, 160, 0)]
                for idx, lm in enumerate(hands):
                    ...
                    cv2.putText(
                        frame, f"Hand {idx + 1}",
                        (pts[0][0] + 8, pts[0][1] - 8), cv2.FONT_HERSHEY_SIMPLEX, 0.6,
                        color, 2, cv2.LINE_AA,
                    )
                ...
                if training:
                    features = get_multi_hand_pairwise_distances(hands)
                    feature_type = _feature_kind(features)
                    ...
                    if pred_class:
                        ...
                        cv2.putText(
                            frame, f"{hand_count} hand(s): no matching samples",
            ...
            if not detection.hand_landmarks:
                with state_lock:
                    if time.time() - state["latest_landmarks_at"] > 2.0:
...
def status():
    with state_lock:
        return jsonify({
            ...
            "classes": _class_counts(state["training_data"]),
            "camera_active": state["camera_active"],
            "k": state["k"],
            "metric": state["metric"],
            "latest_hand_count": len(state["latest_landmarks"]) if state["latest_landmarks"] else 0,
            "latest_landmarks_age": round(time.time() - state["latest_landmarks_at"], 2)
                if state["latest_landmarks"] else None,
            "pending_samples": len(state["pending_samples"]),
        })
...
def load_training():
    ...
    try:
        ...
        return jsonify({"ok": True, "samples": len(td["samples"]), "classes": _class_counts(td), "path": path})
...
def upload_training():
    ...
    try:
        td = json.load(f)
        save_path = _resolve_training_path(f.filename)
        with state_lock:
            state["training_data"] = td
            state["training_path"] = save_path
        return jsonify({"ok": True, "samples": len(td["samples"]), "classes": _class_counts(td), "path": save_path})
    except Exception as e:
        return jsonify({"ok": False, "error": str(e)}), 400
...
def capture_sample():
    ...
    with state_lock:
        ...
        lm_age = time.time() - state["latest_landmarks_at"] if lm_raw else None
    hands = _landmark_payload_to_hands(lm_raw)
    if not hands:
        return jsonify({
            "ok": False,
            "error": "No hand detected. Keep both hands inside the camera frame, then press Capture again.",
            "hand_count": 0,
        }), 400
    if lm_age is not None and lm_age > 2.0:
        return jsonify({
            "ok": False,
            "error": "The last detected hand frame is too old. Move your hands so the skeleton appears again.",
    ...
    sample = {
        ...
        "side": "Both" if len(hands) == 2 else "Unknown",
        "score": 1.0,
        "hand_count": len(hands),
        "feature_type": _feature_kind(features),
    }
...
def commit_gesture():
    ...
    with state_lock:
        ...
        classes = _class_counts(state["training_data"])
    return jsonify({"ok": True, "added": len(added), "total_samples": total, "classes": classes})
...
def save_training():
    ...
    with state_lock:
        ...
        path = _resolve_training_path(requested_path or state["training_path"])
        if state["training_path"] != path:
            state["training_path"] = path
    if td is None:
        return jsonify({"ok": False, "error": "No training data loaded"}), 400
    try:
        ...
        return jsonify({"ok": True, "path": path, "samples": len(td.get("samples", [])), "classes": _class_counts(td)})
    except Exception as e:
...
def delete_gesture():
    ...
    with state_lock:
        ...
        samples = state["training_data"].get("samples", [])
        kept = [s for s in samples if s.get("class") != class_name]
        removed = len(samples) - len(kept)
        if removed == 0:
            return jsonify({"ok": False, "error": f'No samples found for "{class_name}"'}), 404

        state["training_data"]["samples"] = kept
        stats = state["training_data"].get("stats")
        if isinstance(stats, dict):
            stats.pop(class_name, None)
        state["pending_samples"] = [
            s for s in state["pending_samples"] if s.get("class") != class_name
        ]
        total = len(kept)
        classes = _class_counts(state["training_data"])
    return jsonify({
